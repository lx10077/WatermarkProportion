#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Main script for estimating and evaluating the proportion of watermarked content
# using multiple estimators and model sizes across different watermarking methods.
...
# Detection thresholds for different watermarking strategies
...
for watermark in ["Gumbel","Inverse"]:
    for size in ["1p3B", "opt-13b", "Llama-3.1-8B"]:   
        print("========>")
        ...
        for temp in [0.7, 1]:
            ...
            estimation_result_dir = f"fig_c4_data/c4-{watermark}-{size}-N0{N0}-temp{temp}-Iter{use_iterative}.pkl"
            results_dict = dict()
            ...
            true_eps = []
            for eps in all_eps:
                file_dir = f"data/{size}-{watermark}-c4-m500-T500-noncomm_prf-15485863-temp{temp}-eps{eps}.pkl"
            ...
            plt.tight_layout()
            plt.savefig(f"c4-fig-{size}-{watermark}-N0{N0}-temp{temp}-Iter{use_iterative}.pdf", dpi=300)
